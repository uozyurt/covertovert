...
class MyCovertChannel(CovertChannelBase):
    ...
    def send(
        ...
        print(f"Number of packets to send: {number_of_packets_to_send}")
        ...
        for i in range(0, len(number_of_packets_to_send_binary), bits_per_packet):
            # get the bits
            bits = number_of_packets_to_send_binary[i:i+bits_per_packet]
    ...
    def receive(
        ...
        while True:
            ...
            if (timeout_flag):
                ...
                # sniff the packets
                first_packet = scapy.all.sniff(filter=f"udp and dst port {dst_port}", count=1, timeout=timeout_tolerance)

                if len(first_packet) == 0:
                    continue
                
                other_packets = scapy.all.sniff(filter=f"udp and dst port {dst_port}", count=buffer_max_tolerance, timeout=timeout_tolerance*10)
                timeout_flag = True

                packets = first_packet + other_packets
                
                while len(packets) > 0:
                    packet_buffer.append(packets.pop(0))
                ...
                #     packet_buffer.append(packets[i])
            ...
            while len(packet_buffer) > 0:
                ...
                src_port = packet_buffer.pop(0)[scapy.all.UDP].sport
                if verbose >= 1:
                ...
                if verbose >= 2:
                    print(f"Received bits: {bits_buffer[-1]} with source port: {src_port}")
